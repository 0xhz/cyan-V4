    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            ...
            if (interaction.customId === '2313141') {
                ...
                if (qtd > gggaaa.estoque.length) {
                    return interaction.reply({
                        content: `<:cancel:1237422533838635028> A quantidade solicitada de \`${qtd}\` excede o estoque disponível.`,
                        ephemeral: true
                    });
                }
                ...
                if (ggg.cupomadicionado !== undefined) {
                    ...
                    if (gggaaa.condicoes?.precominimo !== undefined) {
                        ...
                        if (qtd < gggaaa.condicoes?.precominimo) {
                            return interaction.reply({ content: `<:cancel:1237422533838635028> A quantidade solicitada de \`${qtd}\` não está no valor minimo para utilizar o cupom de \`${gggaaa.condicoes?.precominimo}\`.`, ephemeral: true })
                        }

                        if (qtd > gggaaa.condicoes?.qtdmaxima) {
                            return interaction.reply({ content: `<:cancel:1237422533838635028> A quantidade solicitada de \`${qtd}\` excede o limite para o uso do cupom de \`${gggaaa.condicoes?.precominimo}\`.`, ephemeral: true })
                        }
        ...
        if (!dbPerms.has(interaction.user.id)) {
            ...
            return;
                ...
                if (carrinhos.has(interaction.channel.id) == false) return interaction.reply({ content: `<:cancel:1237422533838635028> Não há um carrinho aberto neste canal.`, ephemeral: true })
                ...
                const mandanopvdocara = new EmbedBuilder()
                    ...
                    .setTitle(`<:order:1237422817025331372> Pedido solicitado`)
                    .setFooter(
                        { text: interaction.guild.name, iconURL: interaction.guild.iconURL({ dynamic: true }) }
                    )
                    .setTimestamp()
                    .setDescription(`Seu pedido foi criado com sucesso e agora estamos aguardando a confirmação do pagamento`)
                    .addFields(
                ...
                interaction.reply({ content: `<:accept:1237422643544854549> Pagamento aprovado manualmente. Aguarde..`, ephemeral: true })
            ...
            if (interaction.customId.startsWith('comprarid_')) {
                ...
                if (hhhh == null) return interaction.reply({ content: `<:cancel:1237422533838635028> Este produto não existe.`, ephemeral: true }).then(msg => {
                    interaction.message.delete()
                })
            ...
            if (interaction.customId.startsWith('foraestoquealarme_')) {
                ...
                if (gggaaa.avisar !== undefined) {
                    if (!gggaaa.avisar.includes(interaction.user.id)) {
                        ...
                        if (status == 1) {
                            interaction.reply({ content: `<:accept:1237422643544854549> Você será notificado assim que for reabastecido o stock (Para desabilitar clique abaixo).`, ephemeral: true })
                        } else {
                            interaction.update({ content: `<:accept:1237422643544854549> Você será notificado assim que for reabastecido o stock (Para desabilitar clique abaixo).` })
                        }
        ...
        if (Object.keys(infos).length !== 0) {
            ...
            if (gggaaa.condicao?.idcargo !== undefined) {
                ...
                if (temCargo == false) return interaction.reply({ content: `<:cancel:1237422533838635028> Você não possui permissão para comprar esse produto!`, ephemeral: true })
            }
