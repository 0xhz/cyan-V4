    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'ConfigurarPagamentoManual2') {
                ...
                if (a1 !== 'não' && a1 !== 'sim') {
                    return interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Confirmação não validada.` });
                }
                ...
                interaction.followUp({ content: `<:accept:1237422643544854549> Configuração salva.`, ephemeral: true })
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'remcampo') {
                ...
                if (ggg == 0) {
                    interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Esse produto ainda não possuí nenhum campo.` })
                    return
                }
            ...
            if (interaction.customId == 'addcampoo') {
                const ggg2 = await db.get(interaction.message.id)
                const ggg = produtos.get(`${ggg2.name}.Campos`)
                if (ggg.length >= 24) return interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Você já adicionou o máximo de campos possível nesse produto.` })
        ...
        await interaction.update({
          embeds: [],
          components: [],
          content: "<a:1236149701401907210:1237422699740135535> Aguarde...",
        });
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'duigawd8wa8dtgvaw7') {
                ...
                if (confirm !== 'sim') return interaction.reply({ content: `<:cancel:1237422533838635028> Confirmação não validada.`, ephemeral: true })
            ...
            if (interaction.customId === 'sdaju112341111idsjjsdua') {
                ...
                if (isNaN(precocampo)) return interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Preço inserido \`${precocampo}\` inválido.` })
                ...
                interaction.followUp({ content: `<:accept:1237422643544854549> Campo adicionado.`, ephemeral: true })
                await UpdateMessageProduto(client, ggg.name)
            ...
            if (interaction.customId === 'Editar') {
                await interaction.update({ embeds: [], components: [], content: '<a:1236149701401907210:1237422699740135535> Aguarde...' })
            ...
            if (interaction.customId === 'sdaju11111idsjjsdua') {
                ...
                if (produtos.get(`${nome}`) !== null) return interaction.editReply({ content: `<:cancel:1237422533838635028> | Opss, ja existe um produto com esse nome.` })//
...
// <a:1236149701401907210:1237422699740135535> Aguarde...
