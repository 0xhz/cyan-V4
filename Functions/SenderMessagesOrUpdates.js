...
async function MessageCreate(interaction, client) {
    ...
    if (fdfd.textobutton == undefined) {
        ...
        try {
            await interaction.update({ embeds: [], components: [], content: '<a:1236149701401907210:1237422699740135535> Aguarde...' }).then(async msgg => {
                ...
                await channel.send({ embeds: [embed], components: [style2row] }).then(async msggg => {
                    ...
                    await msgg.edit({ content: `<:accept:1237422643544854549> Mensagem postada!`, ephemeral: true, components: [row4] })
            ...
            await interaction.followUp({ content: `<:cancel:1237422533838635028> | Ocorreu um erro ao enviar a mensagem.\n\`${mensagem}\``, ephemeral: true })
        ...
        try {
            await interaction.update({ embeds: [], components: [], content: '<a:1236149701401907210:1237422699740135535> Aguarde...' }).then(async msgg => {
                ...
                await channel.send({ embeds: [embed], components: [row2] }).then(msggg => {
                    ...
                    msgg.edit({ content: `<:accept:1237422643544854549> Mensagem postada!`, ephemeral: true, components: [row4] })
                })
            ...
            interaction.followUp({ content: `<:cancel:1237422533838635028> | Ocorreu um erro ao enviar a mensagem.\n\`${mensagemTraduzida}\``, ephemeral: true })
        }
