    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            ...
            if (interaction.customId == 'sdaju11111231idsj1233js123dua123') {
                ...
                if (tickets.get(`tickets.funcoes.${NOME}`) !== null) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> | Já existe uma função com esse nome!`, ephemeral: true });
                }

                if (NOME.length > 32) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> | O nome não pode ter mais de 32 caracteres!`, ephemeral: true });
                } else {
                    tickets.set(`tickets.funcoes.${NOME}.nome`, NOME)
                }

                if (PREDESC.length > 64) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> | A pré descrição não pode ter mais de 64 caracteres!`, ephemeral: true });
                } else {
                ...
                if (DESC !== '') {
                    if (DESC.length > 1024) {
                        return interaction.reply({ content: `<:cancel:1237422533838635028> | A descrição não pode ter mais de 1024 caracteres!`, ephemeral: true });
                    } else {
                ...
                if (BANNER !== '') {
                    ...
                    if (!urlRegex.test(BANNER)) {
                        ...
                        return interaction.reply({ message: dd, content: `<:cancel:1237422533838635028> | Você escolheu incorretamente a URL do banner!`, ephemeral: true });
                    } else {
                        tickets.set(`tickets.funcoes.${NOME}.banner`, BANNER)
                    }
                }
                ...
                if (EMOJI !== '') {
                    ...
                    if (!emojiRegex.test(EMOJI)) {
                        return interaction.reply({ content: `<:cancel:1237422533838635028> | Você escolheu incorretamente o emoji!`, ephemeral: true });
                    } else {
                ...
                interaction.followUp({ content: `<:accept:1237422643544854549> | Função adicionada com sucesso!`, ephemeral: true });
            ...
            if (interaction.customId == '0-89du0awd8awdaw8daw') {
                ...
                if (TITULO.length > 256) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> | O título não pode ter mais de 256 caracteres!`, ephemeral: true });
                }
                if (DESC.length > 1024) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> | A descrição não pode ter mais de 1024 caracteres!`, ephemeral: true });
                }
                ...
                if (COREMBED !== '') {
                    ...
                    if (!hexColorRegex.test(COREMBED)) {
                        ...
                        return interaction.reply({ content: `<:cancel:1237422533838635028> Código Hex Color \`${COREMBED}\` inváldo, tente pegar [nesse site.](https://www.google.com/search?q=color+picker&oq=color+picker) `, ephemeral: true });
                    }else{
            ...
            if (interaction.customId === 'aslfdjauydvaw769dg7waajnwndjo') {
                ...
                if (CARGO !== '' && VALOR !== '') {
                    ...
                    if (role === null) {
                        return interaction.reply({ content: `<:cancel:1237422533838635028> | Você escolheu incorretamente o ID do cargo!`, ephemeral: true });
                    }

                    if (isNaN(VALOR)) {
                        return interaction.reply({ content: `<:cancel:1237422533838635028> | Você escolheu incorretamente o valor!`, ephemeral: true });
                    }
                ...
                //  interaction.followUp({ content: `<:accept:1237422643544854549> | Posição definida com sucesso!`, ephemeral: true });
        ...
        if (interaction.isChannelSelectMenu()) {
            ...
            if (interaction.customId == 'canalpostarticket') {
                await interaction.reply({ content: `<a:1236149701401907210:1237422699740135535> | Aguarde estamos criando sua mensagem!`, ephemeral: true });
                await CreateMessageTicket(interaction, interaction.values[0], client)
                interaction.editReply({ content: `<:accept:1237422643544854549> | Mensagem criada com sucesso!`, ephemeral: true });
            }
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'sincronizarticket') {
                await interaction.reply({ content: `<a:1236149701401907210:1237422699740135535> | Aguarde estamos atualizando suas mensagem!`, ephemeral: true });
                await Checkarmensagensticket(client)
                interaction.editReply({ content: `<:accept:1237422643544854549> | Mensagens atualizada com sucesso!`, ephemeral: true });
            }


            if (interaction.customId == 'arquivar') {

                if (!interaction.member.roles.cache.has(configuracao.get('ConfigRoles.cargoadm')) && !interaction.member.roles.cache.has(configuracao.get('ConfigRoles.cargosup'))) return interaction.reply({ content: `<:cancel:1237422533838635028> | Você não tem permissão para fazer isso!`, ephemeral: true });
            ...
            if (interaction.customId == `postarticket`) {
                ...
                if (ggg == null || Object.keys(ggg).length == 0 || ggg2 == null || Object.keys(ggg2).length == 0) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> Adicione uma função antes de postar a mensagem.`, ephemeral: true });
                } else {
            ...
            if (interaction.customId == 'remfuncaoticket') {
                ...
                if (ggg == null || Object.keys(ggg).length == 0) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> Não existe nenhuma função criada para remover.`, ephemeral: true });
                }
            ...
            if (interaction.customId.startsWith('addfuncaoticket')) {
                ...
                if (dd && Object.keys(dd).length > 24) {
                    return interaction.reply({ content: `<:cancel:1237422533838635028> | Você não pode criar mais de 24 funções em seu TICKET!` });
                }
