...
async function DentroCarrinhoPix(interaction, client) {
    ...
    await interaction.message.edit({ content: `<a:1236149701401907210:1237422699740135535> Aguarde...`, ephemeral: true, components: [] }).then(async tt => {
        ...
        await mercadopago.payment.create(payment_data)
            ...
            .catch(function (error) {
                ...
                interaction.followUp({ content: `<:message:1237421581647478945> | Ocorreu um erro ao criar o pagamento, tente novamente.\nErro: ${error}`, ephemeral: true })
            })
...
function DentroCarrinho2(interaction) {
    ...
    if (yd.quantidadeselecionada > gggaaa.condicao?.valormaximo) return interaction.reply({ content: `<:cancel:1237422533838635028> | Você não pode comprar mais de \`${gggaaa.condicao.valormaximo}x ${yd.infos.produto} - ${yd.infos.campo}\``, ephemeral: true })
    if (yd.quantidadeselecionada < gggaaa.condicao?.valorminimo) return interaction.reply({ content: `<:cancel:1237422533838635028> | Você não pode comprar mais de \`${gggaaa.condicao.valorminimo}x ${yd.infos.produto} - ${yd.infos.campo}\``, ephemeral: true })
    interaction.deferUpdate()
