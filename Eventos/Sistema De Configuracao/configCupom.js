    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'awdwadpiawjdboawidbae8h6fafah8f') {
                ...
                if (Number.isInteger(qtd)) return interaction.reply({ content: `<:cancel:1237422533838635028> O número de desconto inserio é inválido \`${qtd}\``, ephemeral: true })
                if (dias !== '') {
                    if (Number.isInteger(dias)) return interaction.reply({ content: `<:cancel:1237422533838635028> O número de dias inserio é inválido \`${dias}\``, ephemeral: true })
                ...
                if (produtoExistente) return interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Nome do cupom já existente.` })
                ...
                interaction.followUp({ components: [row3], content: `<:accept:1237422643544854549> Cupom criado.`, ephemeral: true }).then(async msgg => {
            ...
            if (interaction.customId === 'doinabuydoboudwoaynidnioawdawdiaw') {
                ...
                if (cargospodeusar !== '') {
                    ...
                    if (ddd == null) return interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Cargo inserido \`${cargospodeusar}\` inválido.` })
                ...
                interaction.update({ content: `<:accept:1237422643544854549> Alterações bem sucedida` })
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'remcupom') {
                ...
                if (ggg == 0) {
                    interaction.reply({ ephemeral: true, content: `<:cancel:1237422533838635028> Esse produto ainda não possuí nenhum cupom.` })
                    return
                }
