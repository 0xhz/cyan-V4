    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'awdawdawdawdadawdawfewfryty565') {
                ...
                if (a4 !== '') {
                    ...
                    if (!hexColorRegex.test(a4)) return interaction.reply({ content: `<:cancel:1237422533838635028> | Código Hex Color inválido.`, ephemeral: true })
                }
            ...
            if (interaction.customId.startsWith('wdawdawdawdwadadsadawdwadwdw')) {
                ...
                let a1 = interaction.fields.getTextInputValue('tokenMP');
                if (a1 !== 'sim') return interaction.reply({ content: `<:cancel:1237422533838635028> | Ação não validada para realizar reembolso.`, ephemeral: true })
                const id = interaction.customId.split('_')[1]

                await interaction.reply({ content: `<:accept:1237422643544854549> | Estornando pagamento...`, ephemeral: true })
                ...
                interaction.message.edit({ content: `<:accept:1237422643544854549> | Pagamento estornado com sucesso. ( Responsavel: ${interaction.user} )`, components: [] })
                interaction.editReply({ content: `<:accept:1237422643544854549> | Pagamento estornado com sucesso. ( Responsavel: ${interaction.user} )` })
            }
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'syncproduto') {
                ...
                await interaction.reply({ content: `<a:1236149701401907210:1237422699740135535> Sincronizando mensagens...`, ephemeral: true }).then(async msg => {
                    await UpdateMessageProduto(client, ggg.name)

                    msg.edit({ content: `<:accept:1237422643544854549> Mensagens sincronizadas.` })
            ...
            if (interaction.customId == 'colocarvenda') {
                ...
                if (gg2.length == 0) return interaction.reply({ content: `<:cancel:1237422533838635028> | Nenhum campo foi configurado.`, ephemeral: true })
