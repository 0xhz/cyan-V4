            ...
            try {
                if (yy.quantidadeselecionada > 5) {
                    await member.send({
                        ...
                        threadChannel.setName(`<:accept:1237422643544854549>・${yy.user.username}・${yy.user.id}`);
                    })
                ...
                try {
                    setTimeout(async () => {
                        ...
                        if (dd !== null) {
                            ...
                            await member.send({ components: [row6aa], content: `Olá, <@!${member.id}>, deu tudo certo com a sua compra? Não esqueça de nos fazer um feedback! caso ainda não fez.` })
                        }
                    }, 60000);
                } catch (error) {
